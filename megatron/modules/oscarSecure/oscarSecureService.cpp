    ...
    try
    {
        while (1)
        {
            ...
            if (recvd)
            {
                ...
                {
                    MUTEX_INSPECTOR;
                    REF_getter<refbuffer> buf=evt->esi->aes.decrypt(toRef(req));
                ...
                case SB_SET_AES_KEY_C2S:
                {
                    ...
                    {
                        evt->esi->aes.init(evt->esi->rsa.privateDecrypt(b.get_PSTR()));
                ...
                case OscarSecure::SB_HELLO_SSL_S2C:
                {
...

                    {
                        XTRY;
                        XTRY;
                        evt->esi->rsa.initFromPublicKey(pubkey);
                        XPASS;
                        std::string aes_key;
                        XTRY;
                        aes_key=evt->esi->aes.generateRandomKey();
                        evt->esi->aes.init(aes_key);
                        XPASS;
                        outBuffer out;
                        XTRY;
                        out<<evt->esi->rsa.publicEncrypt(aes_key);
...
bool OscarSecure::Service::on_SendPacket(const oscarEvent::SendPacket* e)
{
    ...
    MUTEX_INSPECTOR;
    sendPacketPlain(OscarSecure::SB_SINGLEPACKET_AES,e->esi,e->esi->aes.encrypt(e->buf));
    return true;
}
...
bool OscarSecure::Service::on_Accepted(const socketEvent::Accepted*e)
{
    MUTEX_INSPECTOR;
    XTRY;
    e->esi->rsa.generate_key(RSA_keysize);
    outBuffer o;
    o<<e->esi->rsa.getPublicKey();
    o<<COREVERSION;
